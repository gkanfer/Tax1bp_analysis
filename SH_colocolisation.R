...
out<-"/data/kanferg/temp/SHS/output/"


setwd("/data/kanferg/temp/SHS/")
tif<-dir()[grep(".tiff",dir())]
for (i in 1:length(tif)){
  setwd("/data/kanferg/temp/SHS/")
  frame<-readImage(tif[i],type = "tif",all = T)
  ...
  for (m in 1:dim(stack)[3]){
    ...
    image_name<-paste0(m,"_stack",tif[i])
    meanmcc<-(m1+m2)/2
    df.mcc<-rbind(df.mcc,c(image_name,m1,m2,m1*m2,meanmcc))
    rm(m2,m1,image_name,meanmcc)
    setwd(out)
  ...
  colnames(df.mcc)<-c("image_name","m1","m2","MCC","meanMCC")
  write.table(df.mcc,file = paste0(out,tif[i]),row.names = F)
